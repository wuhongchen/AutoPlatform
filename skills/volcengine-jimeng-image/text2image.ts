  ...
  for (let i = 1; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--debug':
        ...
        break;
      case '--wait':
        download = true; // Wait usually implies getting the result
        break;
    }
  }

  return { prompt, version, ratio, count, width, height, size, seed, outputDir, download, debug, wait: args.includes('--wait') };
}
