...
class Wx:
    ...
    def wxLogin(self, CallBack=None, NeedExit=True):
        ...
        try:
            ...
            print_info("正在加载登录页面...")
            try:
                # networkidle on mp.weixin.qq.com is unstable in headless mode; keep this short and non-blocking.
                page.wait_for_load_state("domcontentloaded", timeout=5000)
                print_info("登录页主文档加载完成")
            except Exception as _exc:
                print_warning("登录页加载等待超时，继续尝试直接抓取二维码")
            ...
            for sel in qr_selectors:
                try:
                    ...
                    qrcode = page.query_selector(sel)
                    if qrcode is not None:
                        break
                except Exception:
                    continue
            if qrcode is None:
                print_warning(f"二维码节点未找到, current_url={page.url}")
                raise Exception("二维码节点不存在，请重试")
            code_src = qrcode.get_attribute("src") or qrcode.get_attribute("data-src")
            if not code_src:
                # 页面初次加载时 src 可能短暂为空，给一个短轮询窗口。
                for _ in range(6):
                    time.sleep(0.5)
                    code_src = qrcode.get_attribute("src") or qrcode.get_attribute("data-src")
                    if code_src:
            ...
            save_dir = os.path.dirname(self.wx_login_url)
            if save_dir and not os.path.exists(save_dir):
                os.makedirs(save_dir, exist_ok=True)
            try:
                page.wait_for_function(
                    "(el) => !!el && (el.complete === undefined || el.complete === true) && "
                    "(el.naturalWidth === undefined || el.naturalWidth > 80)",
                    arg=qrcode,
                    timeout=10000,
                )
            except Exception:
            ...
            for i in range(max_retry):
                ...
                try:
                    if os.path.getsize(self.wx_login_url) > 1200:
                        break
                except Exception:
                    pass
                if i < max_retry - 1:
