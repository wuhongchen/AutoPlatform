...
def test_rewrite_preserves_reusable_original_images(client):
    ...
    # Markdown pipeline: rewritten content is now Markdown (images in original, not auto-merged)
    assert "Mock Rewritten" in rewritten["rewritten_content"]
    assert len(rewritten["markdown_content"]) > 0
...
def test_collected_inspiration_hidden_styles_are_sanitized(client):
    ...
    article = article_resp.get_json()
    # Markdown pipeline: article content is now stored as markdown, original_html is empty
    assert "Visible body text" in article["markdown_content"]
...
def test_manual_article_crud_and_template_preview_with_ads(client):
    ...
    # Markdown pipeline: content stored as markdown, original_html is empty until publish
    assert "第一段" in created["markdown_content"]
    assert "第二段" in created["markdown_content"]
    assert created["original_html"] == ""
    ...
    assert "已更新内容" in updated["markdown_content"]
