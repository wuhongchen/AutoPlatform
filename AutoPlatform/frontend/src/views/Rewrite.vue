      ...
      <el-col :span="16">
        <el-card shadow="never" class="content-card">
          ...
          <template v-else>
            ...
            <div class="article-content" v-html="displayedOriginalContent" />
          </template>
        </el-card>
        ...
        <el-card v-if="result || activeTask" shadow="never" class="result-card">
          ...
          <template v-else>
            <div class="rewritten-content" v-html="displayedRewrittenContent" />
...
• 适合小白阅读"
          ...
          <div v-if="articleId" style="margin-top: 8px">
            <el-button size="small" plain @click="$router.push({ name: 'MarkdownEditor', query: { articleId } })">
              <el-icon><Edit /></el-icon> 在 Markdown 编辑器中打开
            </el-button>
          </div>
        </div>
      </el-col>
...
import { MagicStick, Promotion, CopyDocument, Search, Document, Refresh, PictureFilled, Plus, Delete, Edit } from '@element-plus/icons-vue'
...
// 将 Markdown 内容渲染为 HTML 用于显示
const displayedOriginalContent = computed(() => {
  const md = article.value?.markdown_content || article.value?.original_content || ''
  const html = article.value?.original_html || ''
  return md ? renderMarkdown(md) : html
})

const displayedRewrittenContent = computed(() => {
  const md = article.value?.markdown_content || article.value?.rewritten_content || ''
  const html = article.value?.rewritten_html || ''
  return md ? renderMarkdown(md) : html
})
...
function copyResult() {
  ...
  // 如果内容是 Markdown，渲染为 HTML
  if (!content && (result.value?.markdown_content || article.value?.markdown_content)) {
    return renderMarkdown(result.value?.markdown_content || article.value?.markdown_content)
  }
