...
| 账户 | `/accounts` | 已实现 | 列表+切换 + 创建/编辑/删除 + 账户级广告位配置 |
| 文章 | `/articles` | 已实现 | 状态筛选、后台新建/编辑、改写跳转、发布（带模板选择） |
...
| `POST /api/inspirations/{id}/create-article` | 已实现 | 从素材创建文章 |
| `POST /api/inspirations/{id}/approve` | 兼容保留 | 等价于 create-article |
| `GET /api/articles` | 已实现 | 文章列表（支持 account_id + status 过滤） |
| `POST /api/articles` | 已实现 | 后台手工创建文章 |
| `GET /api/articles/{id}` | 已实现 | 文章详情 |
| `PUT /api/articles/{id}` | 已实现 | 后台编辑文章 |
| `POST /api/articles/{id}/rewrite` | 已实现 | AI 改写（支持 style/use_references/instructions/inspiration_ids） |
| `POST /api/articles/{id}/publish` | 已实现 | 创建发布任务并发布到微信草稿（支持 template） |
...
| `POST /api/pipeline/process` | 兼容保留 | 后端批处理接口仍保留，当前前端暂不暴露 |
| `/admin` | 已实现 | 兼容重定向到 `/` |
...
1. **多账户上下文与全局切换** — Layout 账户选择器 + localStorage 持久化 + 默认回退到首个可用账户。
...
1. **统计返回动态字典** — `GET /api/stats` 按数据库当前值动态聚合，缺固定 schema，缺失状态不返回。
2. **文章后台写稿缺失** — 已补齐 `POST/PUT /api/articles` 与前端文章编辑器。
3. **账户广告位配置缺失** — 已补齐账户级头部 / 底部广告位配置并接入发布链路。
4. **模板移动端可读性一般** — 已补齐统一响应式阅读样式与广告位容器样式。
...
| 文章后台写稿缺失 | 无法直接在后台创作内容 | 新增文章创建/编辑 API 与前端编辑器 | P1 |
| 账户广告位配置缺失 | 无法统一插入公众号头尾广告位 | 账户级配置 + 发布链路注入 | P1 |
| 模板移动端可读性一般 | 手机端阅读体验不稳定 | 增加统一移动端排版与广告位样式 | P1 |
