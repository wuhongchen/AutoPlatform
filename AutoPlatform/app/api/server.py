...
def upload_image_hosting():
    ...
    try:
        ...
        result = service.upload(file_data, filename, mime_type, provider=provider)

        if result.success:
            return jsonify({"url": result.url, "provider": result.provider}), 201
        else:
...
def serve_hosted_image(filename):
    """提供本地图床图片"""
    from app.services.image_hosting import ImageHostingService

    service = ImageHostingService()
    file_data = service.get_local_image(filename)
    ...
    return file_data, 200, {"Content-Type": mime, "Cache-Control": "public, max-age=86400"}
...
def fetch_tech_source():
    """从三方信息源抓取内容并入库为素材"""
    data = request.json or {}
    source_type = data.get("source_type", "")
    if not source_type:
        return jsonify({"error": "source_type 不能为空"}), 400
