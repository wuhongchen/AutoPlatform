...
7. 使用标准 Markdown 格式输出：## 二级标题、### 三级标题、**加粗**、*斜体*、- 无序列表、1. 有序列表、> 引用、![alt](url) 图片、[text](url) 链接。
8. 不要输出 HTML 标签、代码围栏（```）、解释说明或“以下是改写稿”。
9. 段落间用空行分隔，每段 3-5 行。"""
...
9. 使用标准 Markdown 语法：## 二级标题、**加粗**、*斜体*、- 列表、> 引用，段落间空行分隔。
10. 不要输出 HTML 标签、不要输出 \`\`\`、不要输出 html/body/style/script。
...
8. 使用 Markdown 格式输出：## 标题、**加粗**、*斜体*、- 列表等，不要输出 HTML 标签
""")
        try:
    ...
    def _sanitize_rewritten_output(self, text: str) -> str:
        """修复 AI 输出的常见格式问题（Markdown 版本）。
        ...
        - 规范化空行
        """
        from app.services.markdown_utils import strip_markdown_wrapper

        cleaned = strip_markdown_wrapper(text or "")
        if not cleaned:
            return ""

        # 去除残留的 HTML 标签（AI 偶尔仍会输出）
        cleaned = re.sub(r"</?(?:html|body|div|span|section|header|footer|article|style|script)\b[^>]*>", "", cleaned, flags=re.IGNORECASE)
        ...
        for line in lines:
            ...
            if not stripped:
                ...
                continue
            # 跳过推广行
            if self._is_promotional_text(stripped):
                continue
