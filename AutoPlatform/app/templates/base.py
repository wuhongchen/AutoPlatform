...
class BaseTemplate(ABC):
    ...
    def render_from_markdown(self, title: str, markdown_content: str,
                             author: str = "", cover_image: str = "",
                             **kwargs) -> str:
        """
        从 Markdown 内容渲染文章（MD→HTML 转换点）。
        ...
        html_content = markdown_to_html(markdown_content, theme=theme)
        return self.render(title, html_content, author, cover_image, **kwargs)

    def render_fragment_from_markdown(self, title: str, markdown_content: str,
                                      author: str = "", cover_image: str = "",
                                      **kwargs) -> str:
        """
        从 Markdown 内容渲染文章片段（不含完整 HTML 文档结构）。
        ...
        theme = kwargs.get("theme", "default")
        html_content = markdown_to_html(markdown_content, theme=theme)
        return self.render_fragment(title, html_content, author, cover_image, **kwargs)
