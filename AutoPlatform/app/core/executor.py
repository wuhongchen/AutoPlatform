...
class TaskExecutor:
    ...
    def _dispatch_task(self, task):
        ...
        elif name == "tech_fetch":
            return self._run_tech_fetch(payload)
        elif name == "tech_fetch_all":
            return self._run_tech_fetch_all(payload)
        else:
    ...
    def _run_tech_fetch(self, payload: dict):
        """执行三方技术信息源采集"""
        result = self._run_async(
            self._manager.fetch_tech_source(
    ...
    def _run_tech_fetch_all(self, payload: dict):
        """一键采集所有预置技术信息源"""
        result = self._run_async(
            self._manager.fetch_all_tech_sources(
                account_id=payload.get("account_id", "default"),
                limit=int(payload.get("limit", 15)),
            )
        )
