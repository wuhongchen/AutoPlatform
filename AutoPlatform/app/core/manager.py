...
class AppManager:
    ...
    def _normalize_article_content(self, content: str) -> tuple[str, str]:
        """将后台输入规范化为 Markdown 与纯文本。

        Returns:
            (markdown_content, text_content)
        """
        from app.services.markdown_utils import normalize_to_markdown
        ...
        # 统一转为 Markdown
        markdown_content = normalize_to_markdown(raw_content)
    ...
    def render_article_for_wechat_copy(
        ...
        """将文章渲染为可复制到微信公众号编辑器的 HTML。

        Markdown 管道：此处是 MD→HTML 转换点。
        """
        from app.services.markdown_utils import markdown_to_html

        article = self.storage.get_article(article_id)
        if not article:
            raise Exception("文章不存在")

        # 优先使用 Markdown 内容，转为 HTML
        md_content = article.markdown_content or article.rewritten_content or article.original_content or ""
        if not md_content:
    ...
    def create_article_from_inspiration(self, record_id: str) -> Article:
        ...
        # 优先使用 Markdown 内容，回退到纯文本
        markdown_content = record.markdown_content or record.content or ""
        if not markdown_content and record.content_html:
            from app.services.markdown_utils import html_to_markdown
            markdown_content = html_to_markdown(record.content_html)
        ...
        article = Article(
            ...
            original_content=markdown_content,        # 现在存 Markdown
    ...
    def create_manual_article(self, data: Dict) -> Article:
        """后台手工创建文章（内容统一为 Markdown）"""
        title = (data.get("source_title") or data.get("title") or "").strip()
        if not title:
            raise Exception("文章标题不能为空")

        # 优先使用前端传来的 markdown_content，否则从 content 归一化
        markdown_content = (data.get("markdown_content") or "").strip()
        if not markdown_content:
            markdown_content, text_content = self._normalize_article_content(data.get("content", ""))
        else:
            text_content = re.sub(r"[*#>`\-\[\]()!_~|]", "", markdown_content).strip()
        ...
        article = Article(
            ...
            original_content=markdown_content,        # 现在存 Markdown
            original_html="",                          # 发布时再转 HTML
            rewritten_content=markdown_content if publish_ready else "",
            rewritten_html="",                         # 发布时再转 HTML
    ...
    def update_article_content(self, article_id: str, data: Dict) -> Article:
        ...
        if "markdown_content" in data and data["markdown_content"]:
            markdown_content = data["markdown_content"].strip()
        elif data.get("content"):
            markdown_content, _ = self._normalize_article_content(data["content"])
        else:
            markdown_content = article.markdown_content or article.original_content or ""

        if not markdown_content:
            raise Exception("文章内容不能为空")

        publish_ready = bool(data.get("publish_ready"))
        editing_target = (data.get("editing_target") or "original").strip()
        ...
        self.storage.update_article(article_id, {
            ...
            "markdown_content": markdown_content,
            "rewritten_content": markdown_content if publish_ready else article.rewritten_content,
            "status": ArticleStatus.REWRITTEN if publish_ready else ArticleStatus.PENDING,
            "metadata": metadata,
            "error_message": "",
        })
    ...
    async def rewrite_article(
        ...
        try:
            ...
            # Markdown pipeline: AI 返回 Markdown，直接存储
            rewritten_md = result["content"]
            ...
            self.storage.update_article(article_id, {
                "rewritten_content": rewritten_md,
                "rewritten_html": "",              # 发布时再转 HTML
                "markdown_content": rewritten_md,  # 主内容字段
    ...
    def list_tech_source_presets(self) -> Dict:
        """列出所有预置的三方技术信息源"""
        from app.services.tech_sources import TechSourceHub
        return TechSourceHub().list_presets()

    async def fetch_tech_source(self, source_type: str, account_id: str = "default",
                                 limit: int = 15, config: Optional[Dict] = None) -> Dict:
        """从三方技术信息源抓取并入库"""
        from app.services.tech_sources import TechSourceHub

        hub = TechSourceHub()
        articles = hub.fetch(source_type, limit=limit, **(config or {}))

        if not articles:
            return {"source_type": source_type, "collected": 0, "articles": []}

        imported_ids = []
        for article in articles:
            try:
                # 去重
                existing = self.storage.list_inspirations(account_id=account_id, limit=500)
                existing_urls = {r.source_url for r in existing if r.source_url}
                ...
                record = InspirationRecord(
                    ...
                    source_account=article.source_name,
                    title=article.title,
                    author=article.author,
                    summary=article.summary,
                    content=article.summary or article.content_md,
                ...
                logger.warning(f"[tech_fetch] skip article '{article.title[:40]}': {e}")

        logger.info(f"[tech_fetch] {source_type}: imported {len(imported_ids)}/{len(articles)}")
        return {
            "source_type": source_type,
            "collected": len(imported_ids),
            "total": len(articles),
            "article_ids": imported_ids,
        }

    async def fetch_all_tech_sources(self, account_id: str = "default", limit: int = 15) -> Dict:
        """一键采集所有预置技术信息源"""
        from app.services.tech_sources import TechSourceHub

        results = {}
        hub = TechSourceHub()
        for source_type in hub.PRESET_SOURCES:
            try:
                for preset in hub.PRESET_SOURCES[source_type][:1]:  # 每类取 1 个
                    kwargs = dict(preset)
                    kwargs.pop("name", None)
                    kwargs["limit"] = limit
                    result = await self.fetch_tech_source(source_type, account_id, **kwargs)
                    results[f"{source_type}/{preset['name']}"] = result
            except Exception as e:
                logger.warning(f"[tech_fetch_all] skip {source_type}: {e}")

        total = sum(r.get("collected", 0) for r in results.values())
        return {"results": results, "total_collected": total}
    ...
    def generate_article_images(self, article_id: str, slides: List[Dict], insert_into_article: bool = False) -> Dict:
        ...
        if insert_into_article and results:
            md = article.markdown_content or article.rewritten_content or article.original_content or ""
            for img_info in results:
                md_img = f'\n\n![{img_info["name"]}]({img_info["image_url"]})\n\n'
                md += md_img
            self.storage.update_article(article_id, {
                "markdown_content": md,
