...
def _overview(account: Optional[Dict] = None):
    ...
    if not feishu._get_token():
        return {
            "ok": False,
            "error": "飞书鉴权失败，请检查当前账户的 FEISHU_APP_ID / FEISHU_APP_SECRET / FEISHU_APP_TOKEN",
            "account": {
    ...
    pipeline_pref = _safe_text(account.get("feishu_pipeline_table")) or Config.FEISHU_PIPELINE_TABLE
    inspiration_id = _find_table_id(feishu, inspiration_pref, ["内容灵感库"])
    pipeline_id = _find_table_id(feishu, pipeline_pref, ["自动化发布队列", "智能内容库"])

    inspiration_items = feishu.list_records(inspiration_id).get("items", []) if inspiration_id else []
    pipeline_items = feishu.list_records(pipeline_id).get("items", []) if pipeline_id else []

    inspiration_status = {}
    for item in inspiration_items:
        fields = item.get("fields", {})
        st = _safe_text(fields.get("处理状态")) or "(空)"
        inspiration_status[st] = inspiration_status.get(st, 0) + 1

    pipeline_status = {}
    for item in pipeline_items:
        fields = item.get("fields", {})
        st = canonical_pipeline_status(fields.get("数据流程状态"))
        st = _safe_text(st) or "(空)"
        pipeline_status[st] = pipeline_status.get(st, 0) + 1

    recent_inspiration = []
    for item in inspiration_items[:20]:
        f = item.get("fields", {})
        doc_url = _normalize_doc_url(f.get("原文文档")) or _normalize_doc_url(f.get("原文文档链接"))
        attachment = _pick_attachment_meta(f.get("图片") or f.get("素材"))
        recent_inspiration.append({
            "record_id": item.get("record_id"),
            "title": _safe_text(f.get("标题") or f.get("原始标题")),
            "url": _normalize_http_url(f.get("文章 URL")),
            "status": _safe_text(f.get("处理状态")),
            "doc_url": doc_url,
    ...
    recent_pipeline = []
    for item in pipeline_items[:20]:
        f = item.get("fields", {})
        rewritten_doc = _normalize_doc_url(f.get("改后文档链接")) or _normalize_http_url(f.get("改后文档链接"))
        recent_pipeline.append({
            "record_id": item.get("record_id"),
            "title": _safe_text(f.get("标题")),
            "status": _safe_text(canonical_pipeline_status(f.get("数据流程状态"))),
            "model": _safe_text(f.get("改写模型")),
            "url": _normalize_http_url(f.get("文章 URL")),
            "rewritten_doc": rewritten_doc,
            "remark": _safe_text(f.get("备注")),
        })
    ...
    return {
        ...
        "meta": {
            ...
            "pipeline_table_name": pipeline_pref,
            "inspiration_table_id": inspiration_id or "",
            "pipeline_table_id": pipeline_id or "",
            "inspiration_total": len(inspiration_items),
...
def api_pipeline_list():
    ...
    if not table_id:
        return jsonify({
            ...
            "items": [],
            "count": 0,
            "meta": {"table_name": table_name, "table_id": ""},
            "account": {"id": _safe_text(account.get("id")), "name": _safe_text(account.get("name"))},
        })

    raw = feishu.list_records_all(table_id, max_items=limit)
    rows = [_pipeline_row(it) for it in (raw.get("items") or [])]
    rows = _sort_rows_by_time(rows, "updated_at")
    ...
    if keyword:
        rows = [
            ...
            if keyword in _safe_text(it.get("title")).lower()
            or keyword in _safe_text(it.get("url")).lower()
            or keyword in _safe_text(it.get("rewritten_doc")).lower()
            or keyword in _safe_text(it.get("remark")).lower()
            or keyword in _safe_text(it.get("record_id")).lower()
        ]
...
def api_pipeline_preview():
    ...
    table_id = _find_table_id(feishu, table_name, ["自动化发布队列", "智能内容库"])
    if not table_id:
        return jsonify({"ok": False, "error": f"未找到流水线表: {table_name}"}), 404

    record = feishu.get_record(table_id, record_id)
    if not record:
        return jsonify({"ok": False, "error": "record not found"}), 404

    row = _pipeline_row(record)
    ...
    return jsonify({
        ...
        "item": {
            **row,
            ...
            "status": _safe_text(canonical_pipeline_status(fields.get("数据流程状态"))),
...
def api_pipeline_update_status():
    ...
    update_fields = {"数据流程状态": target_status}
    if remark:
        update_fields["备注"] = remark
    elif payload.get("clear_remark") is True:
        update_fields["备注"] = ""

    ok = feishu.update_record(table_id, record_id, update_fields)
    if not ok:
        return jsonify({"ok": False, "error": "更新流水线状态失败"}), 400

    record = feishu.get_record(table_id, record_id) or {"record_id": record_id, "fields": update_fields}
    return jsonify({
        "ok": True,
        "item": _pipeline_row(record),
...
def api_publish_list():
    ...
    rows = _sort_rows_by_time(rows, "published_at")

    if status:
        rows = [it for it in rows if _safe_text(it.get("publish_status")) == status]
    if keyword:
        rows = [
            it for it in rows
            if keyword in _safe_text(it.get("title")).lower()
...
def api_action_inspiration_scan():
    ...
    job_id = _run_command_async(
        ...
        command=[PYTHON_BIN, "core/manager_inspiration.py"],
        account=account,
    )
